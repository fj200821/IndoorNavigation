...
/**
 * Created by javedsalat on 27/08/18.
 *
 * @Capgemini
 */
...
import com.capgemini.vuzixscanner.utils.BlueDotView;
...
public class ImageViewActivity extends FragmentActivity {
    ...
    private double checkoutLatitude=19.17596891666119;
    private double checkoutLongitude=72.990298817438;
    private PreferenceUtils prefUtils ;
    private long mCloseNavigationDelay = 15 * 1000;

    private IALocationListener mLocationListener = new IALocationListenerSupport() {
        @Override
        public void onLocationChanged(IALocation location) {
            Log.d(TAG, "location is: " + location.getLatitude() + ", Longitude is : " + location.getLongitude());

if(location.getLatitude() == checkoutLatitude && location.getLongitude() == checkoutLongitude){
    Toast.makeText(ImageViewActivity.this, "Proceed to Checkout ", Toast.LENGTH_SHORT).show();
}
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        prefUtils = new PreferenceUtils(ImageViewActivity.this);
        ...
        Utility.shareTraceId(findViewById(R.id.imageView), ImageViewActivity.this,
                mIALocationManager);
    ...
    public void completeDelivery(){
        if(!ImageViewActivity.this.isFinishing()) {
            SendDeliveryStatusTask statusTask = new SendDeliveryStatusTask(ImageViewActivity.this);
            statusTask.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
            Utility.showToast(ImageViewActivity.this, "You have reached destination");
        }
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                AlertDialog dialog = new AlertDialog.Builder(ImageViewActivity.this).setMessage("Delivery Complete")
                        .setNeutralButton("Exit", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                Intent dashBoard = new Intent(ImageViewActivity.this,DashBoardActivity.class);
